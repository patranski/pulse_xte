...
void ppolyco(char *dfname,long int imjd_m,double fmjd_m,double *pobs,double *refph)
{
  ...
  char   bpha[10],nsite[2],polyname[32],psrname[12],utdate[9];
...
//  memset(tmpline,0,100);   //
  // memset(polyname,0,17);  // 
  // bzero(tmpline,100);        // Bzero use is deprecated.
  // bzero(polyname,17);
 // 
  //  printf("ncoeff in ppolyco=%d\n",ncoeff);
  ...
  for(i=0; i<8; i++)
    extn[i]=0;
  tmjd = (double)(imjd_m+fmjd_m);
  strncpy(polyname,dfname,9);
  strcpy(extn,".polyco");
...
/*       fgets(tmpline,90,pfile); */
	...
	sscanf(tmpline,"%s %f %lf %f %f\n",utdate,&uttime,&tmid,&dm,&dearth);
      ...
      tmjd1 = tmid - nspan/2880.0;     /* divide nspan by 2 and convert to fractional day*/    // 2880.0 is just two times one day in minutes
      tmjd2 = tmid + nspan/2880.0;
      
      if (tmjd < tmjd1 && loop < 1) {
	printf("%s MJD %lf out of polyco range\n",PPROG,tmjd);
	printf("tmjd=%lf   tmjd1=%lf",tmjd,tmjd1);
	exit(0);
      }
...
//if (verb2) printf("%s Outside Coeff loop phase = %lf pfreq = %.15f\n",PPROG,bphase,pfreq); 
  ...
  for (i=0;i<ncoeff;i++)
      ...
      if (i > 0) 
	{
      ...
      //    if (verb3) printf("%s coeff[%d] = %g change = %12.9f sum = %.7f pfreq = %.15nf\n",
